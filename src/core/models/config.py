...
class ModelConfig(BaseConfig):
    ...
    async def load(
        ...
        # Load model config
        config = await manager.load_config(cls, "model", "model_config.toml")

        return config

    def get_model_path(self, base_path: Path) -> Path:
        """Get the model path."""
        return base_path / self.model.family / f"{self.model.name}-{self.model.size}"

    def get_cache_path(self, cache_dir: Path) -> Path:
        """Get the model cache path."""
        return cache_dir / self.model.family / f"{self.model.name}-{self.model.size}"
