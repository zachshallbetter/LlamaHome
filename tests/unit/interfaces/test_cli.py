...
@pytest.fixture
def cli_interface(mock_request_handler: AsyncMock) -> CLI:
    """CLI interface fixture."""
    return CLI(request_handler=mock_request_handler)


@pytest.mark.asyncio
async def test_cli_lifecycle(cli_interface: CLI) -> None:
...
@pytest.mark.asyncio
async def test_process_prompt(cli_interface: CLI) -> None:
...
@pytest.mark.asyncio
async def test_cli_error_handling(cli_interface: CLI) -> None:
