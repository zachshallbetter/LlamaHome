...
from src.core.config.base import OptimizationConfig, ProcessingConfig, ResourceConfig
from src.core.config.manager import ConfigError, ConfigManager
...
def test_config() -> dict[str, Any]:
    ...
    return {
        ...
        "processing": {"batch_size": 16, "max_sequence_length": 256, "num_workers": 2},
        "optimization": {
            "learning_rate": 1e-4,
            "weight_decay": 0.1,
            "warmup_steps": 50,
...
async def test_load_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_merge_configs() -> None:
    ...
    override = ResourceConfig(gpu_memory_fraction=0.9, cpu_usage_threshold=0.8)
...
async def test_config_manager(
    ...
    resources = await config_manager.load_config(
        ResourceConfig, "resources", "resources_config.toml"
    )
...
async def test_config_updates(config_manager: ConfigManager) -> None:
...
def test_monitoring_config() -> None:
